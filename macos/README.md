...
macOS SDKを使用したコンパイルと実機動作は、この基盤作成時点では未実施です。
...
| メディアスモーク試験 | `tools/media_smoke.mm` | バッファ生成、単一AUデコード |
...
`Normalize720p` はclean apertureまたはpixel aspect ratioのattachmentがあれば明示的に拒否します。実際のデコーダ出力を受ける前に、恒等変換の判定と必要な正規化を追加してください。
