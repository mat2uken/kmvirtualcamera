...
- **手順2の実施（A3）**: `windows/receiver/rtc/peer_connection_manager.*` のReceiverEngine切替を `6cfd7c6` で実施した。pcmのvideo経路（payload/SSRC/キーフレーム/DC仲裁・フレーム配布）を `km::engine::ReceiverEngine` へ集約した。bandwidth estimator・Opus・RTCP flushはpcm側に残した。Windowsはpcmを含む3ターゲットへ `shared/receiver/engine/receiver_engine.cpp` を追加した。Macは `km_rtc_shared` から `km_receiver_engine` をlinkした。`macos/project.yml` のOTHER_LDFLAGSに `-lkm_receiver_engine` を追記した。二重リンクはない。段階3相当の回帰（映像2経路・mid-stream Close、ICE relay-only）を同一SHA `6cfd7c6` で再確認した。詳細は[段階3結果記録](02-03-windows-native.md)のA3節にある。
