...
### M4-a 結果記録（2026-09-24、HEAD `d09a207`・`4347313`・`fc70312`、13コミット先行・未push）
...
| CTestゲート | 成功 | `sh scripts/test_macos_foundation.sh` → 単位1–2で **7/7 pass**、単位3で **8/8 pass**（`receiver_engine` 追加・警告0、ビルド・試験のみ・インストールなし） |

- **主張しない範囲**: 実ブラウザとのsignaling・RTC接続・映像受信は未実施（残り単位とM4-b依存）。本記録はビルドと単体試験のみを根拠とする。
- **記録で延期**: `windows/receiver/rtc/peer_connection_manager.*` のReceiverEngine切替はWindowsビルドでの回帰確認が必要なため後続（M4-b端末依存）へ延期。
- **残り（M4-a）**: AppKit join UI（join URL/QR、`windows/third_party/qr/qrcodegen`）、signaling worker配線、TURN設定の引き継ぎ。
