...
HRESULT WebRtcBridgeActivate::CreateInstance(IMFActivate** ppActivate) {
    ...
    *ppActivate = nullptr;
    ...
    HRESULT hr = MFCreateAttributes(&pActivate->attributes_, 4);
    if (FAILED(hr)) {
        pActivate->Release();
        return hr;
    }
...
WebRtcBridgeActivate::WebRtcBridgeActivate() {
    MFCreateAttributes(&attributes_, 4);
}
...
IFACEMETHODIMP WebRtcBridgeActivate::QueryInterface(REFIID riid, void** ppv) {
    ...
    *ppv = nullptr;
    ...
    } else if (riid == IID_IMFMediaSource || riid == IID_IMFMediaEventGenerator || riid == IID_IMFGetService) {
        std::lock_guard<std::mutex> lock(lock_);
        if (!activeSource_) {
            HRESULT hr = WebRtcBridgeMediaSource::CreateInstance(&activeSource_);
            if (FAILED(hr)) return hr;
        }
        return activeSource_->QueryInterface(riid, ppv);
    } else {
...
IFACEMETHODIMP WebRtcBridgeActivate::GetItem(REFGUID guidKey, PROPVARIANT* pValue) {
    return attributes_ ? attributes_->GetItem(guidKey, pValue) : E_UNEXPECTED;
}
...
IFACEMETHODIMP WebRtcBridgeActivate::GetUINT32(REFGUID guidKey, UINT32* punValue) {
    return attributes_ ? attributes_->GetUINT32(guidKey, punValue) : E_UNEXPECTED;
}

IFACEMETHODIMP WebRtcBridgeActivate::GetUINT64(REFGUID guidKey, UINT64* punValue) {
    return attributes_ ? attributes_->GetUINT64(guidKey, punValue) : E_UNEXPECTED;
}
...
IFACEMETHODIMP WebRtcBridgeActivate::GetGUID(REFGUID guidKey, GUID* pguidValue) {
    return attributes_ ? attributes_->GetGUID(guidKey, pguidValue) : E_UNEXPECTED;
}

IFACEMETHODIMP WebRtcBridgeActivate::GetStringLength(REFGUID guidKey, UINT32* pcchLength) {
    return attributes_ ? attributes_->GetStringLength(guidKey, pcchLength) : E_UNEXPECTED;
}

IFACEMETHODIMP WebRtcBridgeActivate::GetString(REFGUID guidKey, LPWSTR pwszValue, UINT32 cchBufSize, UINT32* pcchLength) {
    return attributes_ ? attributes_->GetString(guidKey, pwszValue, cchBufSize, pcchLength) : E_UNEXPECTED;
}
